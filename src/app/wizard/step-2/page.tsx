...
export type Step2Touched = Array<{ fullName?: boolean; age?: boolean }>;

export default function Step2() {
  const { state, dispatch } = useWizardDraft();
  const router = useRouter();
  const [hasTriedSubmit, setHasTriedSubmit] = useState(false);
  const [touched, setTouched] = useState<Step2Touched>([]);
  ...
  const hasErrors = Boolean(
    errors.travelersCount ||
      (errors.travelers &&
        errors.travelers.some((t) => Boolean(t?.fullName || t?.age)))
  );
  ...
  const visibleTravelerErrors = (errors.travelers ?? []).map((tErr, index) => ({
    fullName: shouldShowTravelerError(index, "fullName")
      ? tErr?.fullName
      : undefined,
  ...
  function handleNext(e: React.FormEvent) {
    ...
    if (hasErrors) {
      const firstBadIndex =
        errors.travelers?.findIndex((t) => Boolean(t?.fullName || t?.age)) ??
        -1;
