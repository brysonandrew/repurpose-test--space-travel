...
describe('Wizard Step 1 - Date Validation', () => {
  it('blocks Next if return date is before departure date', async () => {
    ...
    debug('Before typing: departureInput value:', (departureInput as HTMLInputElement).value);
    await userEvent.clear(departureInput);
    await userEvent.type(departureInput, '2030-01-10');
    debug('After typing: departureInput value:', (departureInput as HTMLInputElement).value);
    debug('Before typing: returnInput value:', (returnInput as HTMLInputElement).value);
    await userEvent.clear(returnInput);
    await userEvent.type(returnInput, '2030-01-05'); // Return before departure
    debug('After typing: returnInput value:', (returnInput as HTMLInputElement).value);
    ...
    debug('Next button disabled after click?', nextBtn.hasAttribute('disabled'));

    // Assert visible error or that navigation did not trigger (no routing change)
    debug('Waiting for return date validation error');
    try {
      expect(screen.getAllByText(/return date must be after/i)[0]).toBeInTheDocument();
