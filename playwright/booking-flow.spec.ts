...
test('happy path booking flow', async ({ page }) => {
  ...
  // Step 2
  await expect(page.getByRole('heading', { name: /^travelers$/i })).toBeVisible();
  ...
  // Step 3
  await expect(page.getByRole('heading', { name: /booking review/i })).toBeVisible();

  const submit = page.getByRole('button', { name: /confirm and book|confirm|submit/i });
